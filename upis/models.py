...
class Korisnik(AbstractBaseUser, PermissionsMixin):
    ...
    tip_korisnika = models.CharField(
        max_length=20, choices=TIP_KORISNIKA, default="user"
    )
...
class Prijava(models.Model):
    korisnik = models.ForeignKey(
        Korisnik, on_delete=models.CASCADE, null=False, blank=False
    )
